...
#Creating a Parsimonious Gaussian Mixture Model(UUU) Alternating Expectation - Conditional Maximization Algorithm:
spatial_pgmmAECM_UUU <- function(x,W,n_images,image_axs_list,image_obs_list,G = 1,Q = 2,epoch=100, convergence_value = 1e-8,zstart=1,zlist=c(),seed=123456){
  #Input:
  #x : Dataframe(N,P). Where n is the number of observations for creating the clusters and p is the features of each observation.
  #epoch : int. Number of iterations.
  #G: int. Number of clusters.
  ...
  if(zstart==3){
     for (g in 1:G){
        #Computing the mean of each class
        mu[g,] = pgmm_result$mu[g,]
        sigma[,,g] = pgmm_result$sigma[,,g]
        psi[,,g] = pgmm_result$psi[,,g]
        lambda[,,g] = pgmm_result$lambda[,,g]
     }
  }
  else{
     for (g in 1:G){
  ...
  for (e in 1:epoch){
    ...
    z_sum=log(apply(exp(z-z_max),1,sum))+z_max
    z_hat=exp(z-z_sum)
    temp_classification=data.frame(z_hat)
    colnames(temp_classification) = c(1:G)
    temp_predicted_cluster = colnames(temp_classification)[max.col(temp_classification, ties.method = "first")]
    cl = unmap(temp_predicted_cluster)
    #Step 1
    #Conditional Maximization
    covw_computed = covw(x,z_hat)
    for (g in 1:G){
    ...
    file_name = paste('originalpuffedcerealfulldata_sPGMM_UUU_G4_epoch_',as.character(save_num),'.RData',sep = '')
    save(result,file = file_name)
    end_time = as.numeric(Sys.time() - start_time,units = "mins")
    #file_name_2 = paste('originalpuffedcerealfulldata_sPGMM_UUU_G4_last_params.RData',sep = '')
    last_params = list(z_map,z_hat,reg_coeff,mu,sigma,tau,Log_Likelihood,lambda,psi,beta,theta,end_time)
    save(last_params,file = 'originalpuffedcerealfulldata_sPGMM_UUU_G4_last_params.RData')
    if(e >= 3){
      log_lik_change = (Log_Likelihood[length(Log_Likelihood)]-Log_Likelihood[length(Log_Likelihood)-1])/(Log_Likelihood[length(Log_Likelihood)-1])
      if(abs(log_lik_change)< convergence_value){
        converged = 'Converged'
...
Image_x_y = list(c(67,53),c(62,64),c(65,55),c(61,34),c(39,69),c(56,54),c(56,49),c(78,54),c(50,44))
obs_list = list(c(1,3551),c(3552,7519),c(7520,11094),c(11095,13168),c(13169,15859),c(15860,18883),c(18884,21627),c(21628,25839),c(25840,28039))
output = spatial_pgmmAECM_UUU(Data,W,9,Image_x_y,obs_list,G = 4,Q = 2,epoch=100000, convergence_value = 1e-7,zstart=1)
save(output, file = 'OriginalPuffedCerealFull_neighbourunscaled_sPGMM_UUU_kmeansstart_pc2_G4_conv1e_7.RData')
...
load('OriginalPuffedCerealFull_neighbourunscaled_sPGMM_UUU_kmeansstart_pc2_G4_conv1e_7.RData')
output$mu
for (g in 1:G){
  z[,g]=log(0.25)+dmvn(as.matrix(data),output$mu[g,],output$sigma[,,g],log=TRUE)
}
z_max = apply(z,1,max)
z_sum=log(apply(exp(z-z_max),1,sum))+z_max
z_hat=exp(z-z_sum)
...
mode_approx = foreach(m = reg_ord_list,.combine = rbind,.export='mode_approx_per_image')%dopar%(mode_approx_per_image(m[[1]],W,z_map,data,output$mu,output$sigma,G,neighbour_pixels_sameclass,neighbour_pixels_differentclass,neighbour_pixels_samedifferentclass,output$reg_coeff))

z_map = mode_approx
neighbour_pixels_sameclass = W%*%z_map
neighbour_pixels_differentclass = (rowSums(neighbour_pixels_sameclass)-neighbour_pixels_sameclass)
neighbour_pixels_samedifferentclass = neighbour_pixels_sameclass - neighbour_pixels_differentclass
tau = spatial_tau_estimation(output$reg_coeff,neighbour_pixels_samedifferentclass,N,G)
#Estimating the probability of each training pixels belonging to each class based on the estimated parameters
for (g in 1:G){
  z[,g]=log(tau[,g])+dmvn(as.matrix(data),output$mu[g,],output$sigma[,,g],log=TRUE)
}
#Computing Log-Sum-Exp
z_max = apply(z,1,max)
z_sum=log(apply(exp(z-z_max),1,sum))+z_max
...
write.csv(uncertainty,file = 'mixture_image_sPGMM_uncertainty.csv')
write.csv(pred_cluster,file = 'mixture_image_sPGMM_predicted_cluster.csv')
temp = read.csv('mixture_image_PGMM_predicted_cluster.csv')
table(temp$x)
