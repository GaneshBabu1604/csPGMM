...
#Creating a Parsimonious Gaussian Mixture Model(UUU) Alternating Expectation - Conditional Maximization Algorithm:
spatial_gmmEM_VVV <- function(x,W,n_images,image_axs_list,image_obs_list,G = 1,Q = 2,epoch=100, convergence_value = 1e-8,zstart=1,zlist=c(),seed=123456){
  #Input:
  #x : Dataframe(N,P). Where n is the number of observations for creating the clusters and p is the features of each observation.
  #epoch : int. Number of iterations.
  #G: int. Number of clusters.
  ...
  for (e in 1:epoch){
    ...
    file_name = paste('originalpuffedcerealfulldata_sGMM_VVV_G4_epoch_',as.character(save_num),'_v2.RData',sep = '')
    save(result,file = file_name)
    end_time = as.numeric(Sys.time() - start_time,units = "mins")
    last_params = list(z_map,z_hat,reg_coeff,mu,sigma,tau,Log_Likelihood,end_time)
    save(last_params,file = 'originalpuffedcerealfulldata_sGMM_VVV_G4_last_params_v2.RData')
    if(e >= 3){
      log_lik_change = (Log_Likelihood[length(Log_Likelihood)]-Log_Likelihood[length(Log_Likelihood)-1])/(Log_Likelihood[length(Log_Likelihood)-1])
      if(abs(log_lik_change)< convergence_value){
        converged = 'Converged'
  ...
  predicted_cluster = colnames(final_prediction)[max.col(final_prediction, ties.method = "first")]
  end_time = as.numeric(Sys.time() - start_time,units = "mins")
  return(list(predicted_cluster=predicted_cluster,initial_cluster = init_cluster, probability_score=z_hat,log_likelihood_list = Log_Likelihood,tau=tau,reg_coeff = reg_coeff,mu=mu,sigma=sigma,converged = converged,bic = bic_model,time = end_time))
}
...
Image_x_y = list(c(67,53),c(62,64),c(65,55),c(61,34),c(39,69),c(56,54),c(56,49),c(78,54),c(50,44))
obs_list = list(c(1,3551),c(3552,7519),c(7520,11094),c(11095,13168),c(13169,15859),c(15860,18883),c(18884,21627),c(21628,25839),c(25840,28039))
output = spatial_gmmEM_VVV(Data,W,9,Image_x_y,obs_list,G = 4,Q = 2,epoch=100000, convergence_value = 1e-8,zstart=1)
save(output, file = 'OriginalPuffedCerealFull_neighbourunscaled_sGMM_VVV_kmeansstart_pc2_G4_conv1e_8.RData')
...
for (g in 1:G){
  z[,g]=log(0.25)+dmvn(as.matrix(data),last_params[[4]][g,],last_params[[5]][,,g],log=TRUE)
}
z_max = apply(z,1,max)
z_sum=log(apply(exp(z-z_max),1,sum))+z_max
z_hat=exp(z-z_sum)
...
last_params[[3]]
mode_approx = foreach(m = reg_ord_list,.combine = rbind,.export='mode_approx_per_image')%dopar%(mode_approx_per_image(m[[1]],W,z_map,data,last_params[[4]],last_params[[5]],G,neighbour_pixels_sameclass,neighbour_pixels_differentclass,neighbour_pixels_samedifferentclass,last_params[[3]]))
...
#Estimating the probability of each training pixels belonging to each class based on the estimated parameters
for (g in 1:G){
  z[,g]=log(tau[,g])+dmvn(as.matrix(data),last_params[[4]][g,],last_params[[5]][,,g],log=TRUE)
}
#Computing Log-Sum-Exp
z_max = apply(z,1,max)
z_sum=log(apply(exp(z-z_max),1,sum))+z_max
...
write.csv(uncertainty,file = 'mixture_image_sGMM_uncertainty.csv')
write.csv(pred_cluster,file = 'mixture_image_sGMM_predicted_cluster.csv')
